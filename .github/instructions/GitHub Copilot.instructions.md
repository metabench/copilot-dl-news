...
- ✅ **Workflow-first gate for V4 crawl incidents**: Before any V4 recovery/TTFSL loop, read in order: `docs/workflows/WORKFLOW_REGISTRY.md` → `docs/workflows/continuous-crawl-repair-loop.md` → `tools/crawl/AGENT.md`. Record the selected workflow path in session `PLAN.md` before issuing operational commands.
- ✅ **Evidence trust model**: Treat trustworthy guidance as broader than official docs. Use canonical/official guidance as baseline, incorporate field-proven community practices when repeatable, and require local metric-based verification before promoting any community pattern into repo policy.

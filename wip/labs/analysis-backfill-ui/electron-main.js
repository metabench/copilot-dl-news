...
// Polyfill File for Electron (Node 18) environment where undici/jsdom might expect it
if (typeof global.File === 'undefined') {
  try {
    // Node 18 has Blob in global or buffer
    const Blob = global.Blob || require('buffer').Blob;
    if (Blob) {
    ...
    console.warn('[electron-main] Failed to polyfill File:', e);
  }
}

const { app: electronApp, BrowserWindow } = require('electron');
const path = require('path');
// const { createAnalysisServer } = require('./analysis-server'); // Lazy loaded below
...
electronApp.whenReady().then(async () => {
  const providedUrl = parseUrlFromArgv();
  let url;

  if (providedUrl) {
    console.log(`[electron-main] Using provided URL: ${providedUrl}`);
    url = providedUrl;
  } else {

...
(function() {
  ...
  const els = {
    ...
    lastResultContent: document.getElementById('last-result-content'),
    optPlaces: document.getElementById('opt-places'),
    optHubs: document.getElementById('opt-hubs'),
    optSignals: document.getElementById('opt-signals'),
    optDeep: document.getElementById('opt-deep')
  };
  ...
  function updateUI(state) {
    ...
    if (state.lastAnalysisResult) {
      ...
      let text = `Kind: ${result.kind || 'unknown'}\n`;
      
      if (result.places && result.places.length) {
        text += `Places (${result.places.length}): ${result.places.map(p => p.place).join(', ')}\n`;
      } else {
        text += `Places: None\n`;
      }
      
      if (result.hubCandidate) {
        text += `Hub Candidate: ${result.hubCandidate.kind} (${result.hubCandidate.confidence})\n`;
        if (result.hubCandidate.placeSlug) text += `  Place: ${result.hubCandidate.placeSlug}\n`;
      }
      
      if (result.deepAnalysis) {
        text += `Deep Analysis: ${Object.keys(result.deepAnalysis).join(', ')}\n`;
      }
      
      els.lastResultContent.textContent = text;
    }
  }
  ...
  els.btnStart.addEventListener('click', async function() {
    try {
      ...
      const analysisOptions = {
        places: els.optPlaces.checked,
        hubs: els.optHubs.checked,
        signals: els.optSignals.checked,
        deep: els.optDeep.checked
      };

...
describe('Test Studio UI visual rerun (Guardian 1000 DB)', () => {
  jest.setTimeout(360_000);

  test('shows UI, runs 1000-page DB crawl via rerun button, and displays passing result', async () => {
    ...
    const artifactDir = path.join(outputRoot, `test-studio-guardian-1000-db-${nowTag}`);
    ...
    try {
      ...
      await page.waitForSelector('#rerun-guardian-1000-db', { timeout: 20000 });
      ...
      await page.click('#rerun-guardian-1000-db');
      ...
      // Evidence: capture the UI while it's actively running.
      const runningStatus = await page.evaluate(() => {
        const el = document.querySelector('#rerun-status');
        return el ? String(el.textContent || '') : '';
      });
      fs.writeFileSync(path.join(artifactDir, 'status-running.txt'), runningStatus, 'utf8');
      await page.screenshot({ path: path.join(artifactDir, 'running.png'), fullPage: true });
      ...
      // Ensure the results list contains our DB persistence test.
      await page.waitForFunction(
        () => {
          const rows = Array.from(document.querySelectorAll('.test-row'));
          return rows.some((r) => (r.textContent || '').includes('downloads and stores 1000 /page/* responses in SQLite'));
        },
        { timeout: 30000 }
      );

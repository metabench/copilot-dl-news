...
The lab uses a split-process architecture to ensure stability and compatibility:

1. **Main Process (Node 25+)**: Runs the `run-lab.js` script and the Analysis Server. This ensures full access to modern Node.js features (like `File`, `fetch`, ESM support) required by JSDOM and other libraries.
2. **Electron Process (Node 18)**: Runs the UI shell. It connects to the server running in the main process.

```
┌─────────────────────────────────────────────────────────────────────┐
│                          Electron App (Node 18)                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  Progress Display (HTML/CSS)                                 │    │
│  │  - Loads http://localhost:3099                               │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                              ▲                                       │
│                              │ SSE events                            │
└──────────────────────────────┼──────────────────────────────────────┘
                               │
┌──────────────────────────────┼──────────────────────────────────────┐
│  Main Process (Node 25+)     │                                       │
│  ┌───────────────────────────┴─────────────────────────────────┐    │
│  │  run-lab.js                                                  │    │
│  │  - Starts Analysis Server (Express)                          │    │
│  │  - Spawns Electron App                                       │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                              │                                       │
│  ┌───────────────────────────┴─────────────────────────────────┐    │
│  │  Analysis Server (port 3099)                                 │    │
...
The UI allows toggling specific analysis components:
- **Places**: Extract geographic locations
- **Hubs**: Identify navigation hubs
- **Signals**: Extract classification signals
- **Deep**: Perform deep analysis (sentiment, etc.)

These options are passed to the backend and respected by the analysis pipeline.
...
2. **Memory on large runs** — For 40k+ pages, consider batching in 5k chunks with restarts

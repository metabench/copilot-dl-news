...
Long-term outcome gate (mandatory). For strategic initiatives expected to span multiple dated sessions, you MUST link work to the single active long-term session under `docs/sessions/long-term/` (or create one if none exists) before implementation. Every tactical session must append milestone deltas to the active long-term session `WORKING_NOTES.md`.

...
class TrendDetector {
  ...
  calculateBaseline(topicId, asOfDate = null) {
    ...
    const { mean, stddev } = pureCalculateBaseline(values);

    return {
      mean,
      stddev: Math.max(stddev, 0.5), // Minimum stddev to avoid division issues
      values
    };
  }
  ...
  calculateTrendScore(currentCount, baseline) {
    const { zScore: score, isTrending, percentChange } = pureCalculateTrendScore(
      currentCount,
      baseline,
      this.sigmaThreshold
    );
    // Pure lib returns object with { zScore, isTrending, percentChange }
    // We map zScore -> score to match legacy API

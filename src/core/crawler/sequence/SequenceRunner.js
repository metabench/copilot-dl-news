...
// Unified SequenceRunner used by both crawler operations and orchestration flows.
// Exposes the same API previously exported from src/orchestration/SequenceRunner.js
// so older imports remain compatible via re-export.
...
const createSequenceRunner = ({
  ...
  const normalizeStepOverrideInput = (stepOverrides = {}, step, index) => {
    ...
    if (stepOverrides[step.id]) return stepOverrides[step.id];
    if (stepOverrides[step.operation]) return stepOverrides[step.operation];
    if (Object.prototype.hasOwnProperty.call(stepOverrides, index)) return stepOverrides[index];
    return undefined;
  };

 ...
 */
function assessDomainReadiness({ domain, kinds, metrics = {}, dsplEntry = null, latestDetermination = null, minPageCount = 500 } = {}) {
  const assessment = {
    domain: domain.host,
    ready: false,
    reasons: [],
  ...
  // Check page count
  const distinctUrlCount = metrics.fetchCount || 0;
  if (distinctUrlCount < minPageCount) {
    assessment.reasons.push(`Insufficient crawled pages: ${distinctUrlCount} < ${minPageCount}`);
    assessment.recommendations.push(`Crawl at least ${minPageCount - distinctUrlCount} more pages`);
  }

...
describe('ensureDatabase fast path', () => {
  ...
  test('records schema fingerprint after initialization and uses fast path on reuse', () => {
    ...
    const verificationDb = openNewsCrawlerDb(dbPath, { readonly: true, fileMustExist: true });
  ...
  test('fallbacks to full initialization when critical table is missing', () => {
    ...
    const raw = openNewsCrawlerDb(dbPath);
    ...
    const verifyDb = openNewsCrawlerDb(dbPath, { readonly: true, fileMustExist: true });

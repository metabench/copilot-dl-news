 ...
 * 
 * Originally extracted from docsViewer/utils/svgRenderer.js
 */
 ...
 * @param {string} containerClass - CSS class for container (default: "svg-container")
 * @returns {jsgui.Control} - Container with SVG
 */
function renderSvgContent(context, svgContent, filename, containerClass = "svg-container") {
  const container = new jsgui.Control({ context, tagName: "div" });
  container.add_class(containerClass);
  
  const wrapper = new jsgui.Control({ context, tagName: "div" });
  wrapper.add_class(`${containerClass}__wrapper`);
  ...
  try {
    ...
    errorMsg.add_class(`${containerClass}__error`);
  ...
  caption.add_class(`${containerClass}__filename`);

...
function formatDuration(ms) {
  ...
  if (value < 1000) return `${value | 0}ms`;
...
function renderAnalysisListPage({ runs, total, limit, renderNav }) {
  ...
  const rowsHtml = runs.length
    ...
    : '<tr><td colspan="7" class="meta">No analysis runs yet.</td></tr>';
...
<style>
  ...
  <div class="container">
    ...
    <form class="filters" method="GET" action="/analysis/ssr">
      <label>Limit <input type="number" min="1" max="200" name="limit" value="${safeLimit}"/></label>
      <button type="submit">Apply</button>
      <span class="meta">Total ${escapeHtml(total)}</span>
    </form>
    <table>

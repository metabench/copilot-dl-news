...
function createResumeAllRouter(options = {}) {
  ...
  // Initialize QueuePlannerService
  ...
  router.get('/api/resume-all', (req, res, next) => {
    ...
    try {
      ...
      // Use service or fallback to crawlerManager
      const { runningJobIds, runningDomains } = crawlerManager
        ? crawlerManager.collectRunningContext()
        : queuePlanner.collectRunningContext(jobRegistry);
      ...
      const summary = crawlerManager
        ...
        : queuePlanner.buildQueueSummary(plan, { now });
  ...
  router.post('/api/resume-all', async (req, res, next) => {
    const MAX_CONCURRENT = 8;

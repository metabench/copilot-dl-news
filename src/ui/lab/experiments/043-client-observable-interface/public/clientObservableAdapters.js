/* global createRemoteObservableClient */
"use strict";
...
(function factory(root) {
  ...
  // 1) Evented_Class-style adapter (jsgui3 controls use .on/.off/.raise)
  ...
  function toAsyncIterator(connection, { yieldMode = 'next' } = {}) {
    ...
    const onError = (err) => {
      ...
      if (pendingResolve) {
        const r = pendingResolve;
        pendingResolve = null;
        // Reject by throwing on next pull.
        r({ value: undefined, done: true });
      }
    };
  ...
  root.RemoteObservableClientAdapters = {

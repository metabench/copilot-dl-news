...
class CrawlProgressDemo extends Control {
	constructor(spec = {}) {
		...
		if (!spec.el) {
			...
			// Same-origin SSE endpoint (hosted by jsgui3-server itself).
			const sseUrl = "/events";
	...
	compose() {
		...
		title.add_text("jsgui3-server /events (SSE) → CrawlDisplayAdapter → ProgressBar");
...
class CrawlProgressPage extends Active_HTML_Document {
	...
	compose() {
		...
		title.add_text("jsgui3-server SSE + Telemetry (Lab 028)");

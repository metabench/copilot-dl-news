...
/**
 * Creates a jobs manager responsible for rendering current crawl jobs
 * and syncing stage/paused indicators while keeping DOM access localized.
 */
function createJobsManager({
  elements = {},
  actions = {},
  ...
  function renderJobs(jobs) {
    try {
      ...
      each(jobs.items, (job) => {
        ...
        const downloaded = job.download ?? job.downloaded ?? 0;
      ...
      // Rendering resilience matters more than bubbling errors here.
    } finally {
      if (jobsList) {
        jobsList.setAttribute("aria-busy", "false");
      }
  ...
  async function initialJobsPoll() {
    try {
      ...
      // Swallow network failures; SSE will backfill data later.
    }
  }

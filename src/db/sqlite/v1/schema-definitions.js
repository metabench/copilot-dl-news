 ...
 * Generated at 2026-01-07T19:14:51.100Z
 *
...
const TABLE_STATEMENTS = [
  ...
  `CREATE TABLE IF NOT EXISTS content_analysis (
    ...
    , confidence_score REAL, body_text TEXT, byline TEXT, authors TEXT)`,
  ...
  // crawl_jobs
  `CREATE TABLE IF NOT EXISTS "crawl_jobs" (id TEXT PRIMARY KEY, args TEXT, pid INTEGER, started_at TEXT, ended_at TEXT, status TEXT, crawl_type_id INTEGER, url_id INTEGER, paused_at TEXT, pause_reason TEXT, resume_at TEXT, pause_count INTEGER DEFAULT 0, total_paused_ms INTEGER DEFAULT 0)`,
  ...
  `CREATE TABLE IF NOT EXISTS news_websites (
        ...
        enabled INTEGER DEFAULT 1,
        metadata TEXT
      , favicon_data TEXT, favicon_content_type TEXT, favicon_updated_at TEXT, favicon_fetch_error TEXT)`,
  ...
  `CREATE TABLE IF NOT EXISTS place_page_mappings (
      ...
      verified_at TEXT,
      evidence JSON,
      hub_id INTEGER, max_page_depth INTEGER, oldest_content_date TEXT, last_depth_check_at TEXT, depth_check_error TEXT,
  ...
  // topic_page_mappings
  `CREATE TABLE IF NOT EXISTS topic_page_mappings (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      topic_slug TEXT NOT NULL,
      host TEXT NOT NULL,
      url TEXT NOT NULL,
      page_kind TEXT NOT NULL DEFAULT 'topic-hub',
      publisher TEXT,
      status TEXT NOT NULL DEFAULT 'pending',
      first_seen_at TEXT NOT NULL DEFAULT (datetime('now')),
      last_seen_at TEXT NOT NULL DEFAULT (datetime('now')),
      ...
      oldest_content_date TEXT,
      last_depth_check_at TEXT,
      depth_check_error TEXT,
      UNIQUE(topic_slug, host, page_kind)
    )`,
...
const INDEX_STATEMENTS = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_crawl_jobs_paused 
  ...
  `CREATE INDEX IF NOT EXISTS idx_crawl_pause_history_job
  ...
  `CREATE INDEX IF NOT EXISTS idx_crawl_queue_updated
        ON crawl_queue (updated_at) WHERE status = 'in-progress'`,
  `CREATE INDEX IF NOT EXISTS idx_crawl_queue_worker 
  ...
  `CREATE INDEX IF NOT EXISTS idx_domain_rate_events_domain_time 
    ON domain_rate_events(domain, created_at DESC)
  `,
  `CREATE INDEX IF NOT EXISTS idx_domain_rate_limits_updated 
  ...
  `CREATE INDEX IF NOT EXISTS idx_places_kind_population ON places(kind, population DESC)`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_robots_cache_crawl_delay 
    ON robots_cache(domain) WHERE crawl_delay_seconds IS NOT NULL
  `,
  `CREATE INDEX IF NOT EXISTS idx_robots_cache_expires 
  ...
  `CREATE UNIQUE INDEX IF NOT EXISTS uniq_country_places ON places(country_code) WHERE kind='country' AND country_code IS NOT NULL`,
  `CREATE UNIQUE INDEX IF NOT EXISTS uniq_crawl_skip_terms ON crawl_skip_terms(lang, normalized)`,
  `CREATE UNIQUE INDEX IF NOT EXISTS uniq_place_names ON place_names(place_id, normalized, lang, name_kind)`,
  `CREATE UNIQUE INDEX IF NOT EXISTS uniq_place_sources ON place_sources(name, version, url, license)`,
  `CREATE UNIQUE INDEX IF NOT EXISTS uniq_region_places ON places(country_code, adm1_code) WHERE kind='region' AND country_code IS NOT NULL AND adm1_code IS NOT NULL`,

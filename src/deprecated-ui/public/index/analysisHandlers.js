/**
 * @file analysisHandlers.js
 * @description Handles analysis progress, planner stage events, and milestone tracking.
...
/**
 * Creates analysis event handlers
 * @param {Object} deps - Dependencies
 * @param {Object} deps.appActions - Application actions (pushMilestone, recordPatternEvent, patchPipeline, etc.)
 * @param {Object} deps.pipelineControl - Pipeline control API (getAnalysisState, renderAnalysisHistory, etc.)
 * @param {Function} deps.setCrawlType - Function to set crawl type badge
 * @param {Function} deps.renderAnalysisLink - Function to render analysis detail link
 * @param {Function} deps.renderAnalysisStatus - Function to render analysis status panel
 * @param {Function} deps.refreshCoverageMetric - Function to refresh coverage metric display
 * @param {Function} deps.getCurrentCrawlType - Function to get current crawl type
 * @param {HTMLElement} deps.analysisStatus - Analysis status element (for metrics rendering)
 * @returns {Object} Analysis handlers API
 */
export function createAnalysisHandlers(deps) {
  ...
  /**
   * Handles milestone events (patterns-learned, intelligent-completion)
   * @param {Object} m - Milestone event object
   */
  ...
  /**
   * Handles analysis progress updates
   * @param {Object} payload - Analysis progress payload
   */
  function handleAnalysisProgress(payload) {
    ...
    // Handle run ID and detect new runs
    ...
    // Render metrics if status element exists
    if (analysisStatus) {
      const metrics = [];
    ...
    // Update detail link and localStorage
    if (detailUrl) {
    ...
    // Update insights if details present
  ...
  /**
   * Handles planner stage events
   * @param {Object} ev - Planner stage event object
   */
  ...
  /**
   * Updates intelligent insights with new details
   * @param {Object} details - Insight details
   * @param {Object} extras - Extra metadata (source, timestamp, pipelinePatch, etc.)
   */

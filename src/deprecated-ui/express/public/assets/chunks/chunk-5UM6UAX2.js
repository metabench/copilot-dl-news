...
var require_themeController = __commonJS({
  "src/ui/shared/theme/themeController.js"(exports, module) {
    var { each: each2, tof } = require_lang();
    var DEFAULT_THEMES2 = Object.freeze([
      { name: "light", classes: [] },
    ...
    function normaliseThemeName(name, themes, registry) {
      ...
      const map = registry || buildRegistry(themes || DEFAULT_THEMES2);
    ...
    function createThemeController2(options = {}) {
      const {
        themes = DEFAULT_THEMES2,
        storageKey = "theme",
        defaultTheme = "light",
    ...
    module.exports = {
      DEFAULT_THEMES: DEFAULT_THEMES2,
      createThemeController: createThemeController2,
      parseThemeFromHash,
      resolveThemeName,
      normaliseThemeName
    };
...
var import_themeController = __toESM(require_themeController());
function createBrowserThemeController(options = {}) {
  ...
  const {
    themes = import_themeController.DEFAULT_THEMES,
    storageKey = "theme",
    hashParam = "theme_name",
    defaultTheme = "light",
    fallbackTheme = themes[0] ? themes[0].name : "light",
  ...
  const controller = (0, import_themeController.createThemeController)({
    themes,
    storageKey,
    hashParam,
    defaultTheme,
    ...
    onThemeApplied: ({ theme, classes, allClasses }) => {
      ...
      (0, import_lang_tools.each)(allClasses, (cls) => {
        if (cls) {
          root.classList.remove(cls);
        }
      });
      (0, import_lang_tools.each)(classes, (cls) => {
        if (cls) {

...
function createMetricsView({ elements = {}, formatNumber }) {
  const {
    reqpsLabel,
    dlpsLabel,
    errpm,
  ...
  function scheduleQueueUpdate(rawValue) {
    ...
    const effective = queueState.lastStable != null ? queueState.lastStable : queueState.lastRaw != null ? queueState.lastRaw : null;
    if (effective == null) return null;
    const shouldRender = queueState.lastRenderedAt === 0 || now - queueState.lastRenderedAt >= 400 || queueState.lastRenderedValue !== effective;
    if (shouldRender) {
  ...
  function updateQueueSparkline(series) {
    ...
    qPoly.classList.toggle("spark-bad", last > 2e3);
    qPoly.classList.toggle("spark-warn", last > 500 && last <= 2e3);
  ...
  function updateDownloadMetrics(now, queueSize) {
    ...
    const windowSeconds = Math.max(1e-3, Math.min(15, (now - firstSampleTime) / 1e3));
    ...
    if (dlpsLabel) {
      dlpsLabel.textContent = `dl/s (\u226415s avg): ${dlps.toFixed(2)} \xB7 MB/s (\u226415s avg): ${mbs.toFixed(2)}`;
    }
  ...
  function handleProgress(progress, now = Date.now()) {
    ...
    const dt = Math.max(1e-3, (safeNow - lastProgress.t) / 1e3);
...
interval: ~${progress.domainIntervalMs}ms` : `limit per minute: ${progress.domainLimit}`;
    ...
    if (progress.domainBackoffMs != null && domBk) {
      ...
      const secondsLeft = Math.ceil((progress.domainBackoffMs || 0) / 1e3);
      if (secondsLeft > 0) {
    ...
    const cutoff = safeNow - 5 * 60 * 1e3;
    trafficSamples = trafficSamples.filter((sample) => sample.t >= cutoff);
    if (cacheGauge) {
      const ratio1m = summarizeCacheRatio(60 * 1e3, safeNow);
      const ratio5m = summarizeCacheRatio(5 * 60 * 1e3, safeNow);
      cacheGauge.textContent = `cache: 1m ${(ratio1m * 100).toFixed(0)}% \xB7 5m ${(ratio5m * 100).toFixed(0)}%`;
    }
  ...
  async function refreshServerMetrics(fetchFn = fetch) {
    try {
      ...
      if (queueSizeMetric != null) {
        ...
        qsize?.classList.toggle("warn", queueSizeMetric > 500 && queueSizeMetric <= 2e3);
        qsize?.classList.toggle("bad", queueSizeMetric > 2e3);
      }
  ...
  function startServerMetricsPolling(intervalMs = 2e3, fetchFn) {
    return setInterval(() => {
      refreshServerMetrics(fetchFn);
    }, intervalMs);
  }
  function startDlpsTicker(intervalMs = 1e3) {
    return setInterval(() => {

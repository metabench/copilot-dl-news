...
function buildRegistry(themes) {
  ...
  (0, import_lang_tools.each)(themes, (theme) => {
    if (!theme || !theme.name) {
      return;
    }
...
function createThemeController(options = {}) {
  ...
  const callListeners = (payload) => {
    (0, import_lang_tools.each)(listeners, (listener) => {
      try {
        listener(payload);
      } catch (err) {
      }
  ...
  const apply = (themeName, { persist = true, source = "manual" } = {}) => {
    ...
    if (persist && (0, import_lang_tools.tof)(writeStorage) === "function") {
      try {
    ...
    if ((0, import_lang_tools.tof)(onThemeApplied) === "function") {
      try {
  ...
  const init = () => {
    ...
    if ((0, import_lang_tools.tof)(readStorage) === "function") {
      try {
  ...
  const clearStoredTheme = () => {
    if ((0, import_lang_tools.tof)(removeStorage) === "function") {
      try {
...
function createBrowserThemeController(options = {}) {
  ...
  const controller = createThemeController({
    ...
    onThemeApplied: ({ theme, classes, allClasses }) => {
      ...
      (0, import_lang_tools2.each)(allClasses, (cls) => {
        if (cls) {
          root.classList.remove(cls);
        }
      });
      (0, import_lang_tools2.each)(classes, (cls) => {
        if (cls) {

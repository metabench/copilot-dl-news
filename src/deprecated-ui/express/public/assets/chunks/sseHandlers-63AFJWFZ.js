...
`;
      ...
      if (payload.gazetteer && progress) {
        ...
        if (gaz.payload) {
          const p = gaz.payload;
          if (p.phase === "processing" && (0, import_lang_tools.is_defined)(p.current) && (0, import_lang_tools.is_defined)(p.totalItems)) {
            const percent = Math.round(p.current / p.totalItems * 100);
            progressText = `Processing: ${p.current}/${p.totalItems} (${percent}%) ${p.message || ""}`;
          } else if (p.phase === "discovery-complete" && (0, import_lang_tools.is_defined)(p.totalItems)) {
            ...
            progressText = `${phase} \xB7 ${p.phase || ""}`;
          }
        } else if (gaz.status) {

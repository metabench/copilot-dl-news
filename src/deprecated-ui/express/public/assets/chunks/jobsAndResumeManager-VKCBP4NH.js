  ...
  function renderJobs(jobs) {
    try {
      ...
      (0, import_lang_tools.each)(jobs.items, (it) => {
        ...
        const act = it.lastActivityAt ? ` \xB7 active ${Math.round((Date.now() - it.lastActivityAt) / 1e3)}s ago` : "";
        const pid = it.pid ? ` \xB7 pid ${it.pid}` : "";
        ...
        const statusMeta = statusLine.length ? ` \xB7 ${statusLine.join(" \xB7 ")}` : "";

...
function createJobsAndResumeManager(deps) {
  ...
  function renderResumeInventory(data) {
    ...
    if ((0, import_lang_tools.is_defined)(elements.resumeAllBtn)) {
      ...
      elements.resumeAllBtn.textContent = recommendedCount ? `Resume ${recommendedCount} queue${recommendedCount === 1 ? "" : "s"}` : "Resume suggestions";
    }
  ...
  function setupResumeControls() {
    ...
    if ((0, import_lang_tools.is_defined)(elements.clearQueuesBtn)) {
      elements.clearQueuesBtn.addEventListener("click", async () => {
        if (!confirm("Clear all paused queues? This cannot be undone.")) {
          return;
        }
        ...
        try {
          ...
          if (response.ok) {
            logs.info(`Cleared ${data.deleted} queue${data.deleted === 1 ? "" : "s"}`);
            fetchResumeInventory();
          } else {
            logs.error(`Failed to clear queues: ${data.message || "Unknown error"}`);
          }
        } catch (err) {
          logs.error(`Error clearing queues: ${err.message}`);
        } finally {
          elements.clearQueuesBtn.disabled = false;
          elements.clearQueuesBtn.textContent = "Clear Queues";
        }
      });
    }
    if ((0, import_lang_tools.is_defined)(elements.resumeAllBtn)) {

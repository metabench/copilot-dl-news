...
// src/ui/public/index/pipelineView.js
...
function describeAnalysisHistoryEntry(entry) {
  ...
  return parts.length ? parts.join(" \xB7 ") : "Analysis update";
}
...
function createPipelineView(dom, formatters = {}) {
  const {
    panel,
    updatedEl,
    cards = {},
  ...
  const {
    formatNumber: formatNumber2 = formatNumber,
    formatRelativeTime: formatRelativeTime2 = formatRelativeTime
  } = formatters;
  const state = {
  ...
  function render() {
    ...
    for (const [stageKey, stage] of Object.entries(state)) {
      ...
      if (stageKey === "analysis") {
        if (els.link) {
          ...
          const runId = stage.runId || stage.runID || stage.run || void 0;
        ...
        if (els.lastRun) {
          if (stage.lastRun) {
            ...
            if (Number.isFinite(ts)) {
              els.lastRun.textContent = formatRelativeTime2(ts);
        ...
        if (els.signals) {
          ...
          if (signals.length) {
            els.signals.textContent = signals.slice(0, 3).join(" \xB7 ");
        ...
        if (els.goals) {
          const goalSummary = stage.goalSummary || (stage.goals && stage.goals.total ? `${formatNumber2(stage.goals.completed || 0)}/${formatNumber2(stage.goals.total)} complete` : "\u2014");
        ...
        if (els.jobs) {
          ...
          els.jobs.textContent = Number.isFinite(jobsVal) ? formatNumber2(jobsVal) : String(stage.jobs ?? "0");
        }
        if (els.queue) {
          ...
          if (queueVal == null) {
            ...
            els.queue.textContent = Number.isFinite(num) ? formatNumber2(num) : String(queueVal);
    ...
    if (updatedEl) {
      if (hasActivity && latestTs) {
        updatedEl.textContent = `Updated ${formatRelativeTime2(latestTs)}`;
      } else if (hasActivity) {
  ...
  function applyStagePatch(stageKey, patch, safeTs) {
    ...
    for (const [prop, value] of Object.entries(patch)) {
      ...
      if (value && (0, import_lang_tools.tof)(value) === "object" && !(0, import_lang_tools.is_array)(value)) {
        const existing = target[prop] && (0, import_lang_tools.tof)(target[prop]) === "object" && !(0, import_lang_tools.is_array)(target[prop]) ? target[prop] : {};
        const merged = { ...existing, ...value };
        if (JSON.stringify(existing) !== JSON.stringify(merged)) {
          target[prop] = merged;
          stageChanged = true;
        }
      } else if ((0, import_lang_tools.is_array)(value)) {
        const existing = (0, import_lang_tools.is_array)(target[prop]) ? target[prop] : [];
    ...
    if (stageChanged || patch.updatedAt != null) {
      const patchTs = patch.updatedAt != null ? typeof patch.updatedAt === "number" ? patch.updatedAt : Date.parse(patch.updatedAt) : safeTs;
  ...
  function updatePipeline(partial = {}, options = {}) {
    let changed = false;
    const baseTs = options.timestamp != null ? typeof options.timestamp === "number" ? options.timestamp : Date.parse(options.timestamp) : Date.now();
  ...
  function persistAnalysisHistory(history) {
    try {
      const trimmed = Array.isArray(history) ? history.filter((entry) => entry && Number.isFinite(entry.ts)).slice(-50) : [];
  ...
  function renderAnalysisHistory(entries, options = {}) {
    ...
    const list = Array.isArray(entries) ? entries.slice().sort((a, b) => (b?.ts || 0) - (a?.ts || 0)) : [];
    if (!list.length) {
    ...
    for (let i = 0; i < limit; i += 1) {
      ...
      if (!Number.isNaN(dt.getTime())) {
        ...
        timeEl.textContent = formatRelativeTime2(ts);
  ...
  function buildAnalysisHighlights(stateSnapshot = {}) {
    ...
    if (coverage && coverage.expected != null) {
      const expected = formatNumber2(coverage.expected);
      const seededCount = coverage.seeded != null ? formatNumber2(coverage.seeded) : null;
      const pct = coverage.coveragePct != null ? coverage.coveragePct > 1 ? coverage.coveragePct : coverage.coveragePct * 100 : null;
      ...
      highlights.push(parts.join(" \xB7 "));
    }
    if (seeded) {
      if (seeded.sectionsFromPatterns != null && seeded.sectionsFromPatterns > 0) {
        highlights.push(`${formatNumber2(seeded.sectionsFromPatterns)} sections inferred from patterns`);
      }
      if (seeded.countryCandidates != null && seeded.countryCandidates > 0) {
        highlights.push(`${formatNumber2(seeded.countryCandidates)} country hub candidates queued`);
      }
    ...
    if (Array.isArray(stateSnapshot.problems) && stateSnapshot.problems.length) {
      ...
      if (top && top.kind) {
        highlights.push(`Needs attention: ${top.kind} (${formatNumber2(top.count || 0)})`);
      }
    }
    if (stats && (stats.articlesSaved != null || stats.articlesFound != null)) {
      const saved = formatNumber2(stats.articlesSaved || 0);
      const found = formatNumber2(stats.articlesFound || 0);

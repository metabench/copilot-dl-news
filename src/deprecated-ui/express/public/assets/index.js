// src/ui/public/index.js
document.addEventListener("DOMContentLoaded", async () => {
  try {
    let renderAnalysisStatus2 = function(summary, options = {}) {
    ...
    }, refreshCoverageMetric2 = function() {
      if (!metricCoverage) return;
      const summary = uiApp.getState().cache?.coverageSummary;
      metricCoverage.textContent = summary && typeof summary.value === "number" ? `${summary.value.toFixed(0)}%` : "\u2014";
    }, renderStructureSummary2 = function(structure) {
    ...
    }, resetInsights2 = function(hint = "Insights appear once planner telemetry streams in.") {
      appActions.resetInsights(hint);
      appActions.resetPatternInsights();
      refreshCoverageMetric2();
    }, setCrawlType2 = function(type) {
      ...
      if (norm === "intelligent") {
        ...
        if (structurePanel) {
          ...
          renderStructureSummary2(lastStructureSummary);
        }
      } else {
        crawlTypeBadge.classList.add("badge-basic");
        if (insightsPanel && insightsPanel.dataset.hasData !== "1") hideElement(insightsPanel);
        if (!window.__coverageSummary) refreshCoverageMetric2();
        if (structurePanel) {
          structurePanel.dataset.active = "0";
          hideElement(structurePanel);
        }
      }
    }, attachJsonEventListener2 = function(eventSource, eventName, handler) {
    ...
    }, attachCrawlProgressListeners2 = function(eventSource) {
      if (!eventSource) return;
      attachJsonEventListener2(eventSource, "progress", (data) => {
        crawlProgress.handleProgress(data);
      });
      attachJsonEventListener2(eventSource, "telemetry", (data) => {
        crawlProgress.handleTelemetry(data);
      });
      attachJsonEventListener2(eventSource, "milestone", (data) => {
        crawlProgress.handleMilestone(data);
      });
      attachJsonEventListener2(eventSource, "queue", (data) => {
        crawlProgress.handleQueue(data);
      });
      attachJsonEventListener2(eventSource, "task-progress", (task) => {
      ...
      attachJsonEventListener2(eventSource, "done", (data) => {
        crawlProgress.handleCrawlComplete(data);
      });
      attachJsonEventListener2(eventSource, "error", (data) => {
        crawlProgress.handleCrawlError(data);
      });
      attachJsonEventListener2(eventSource, "planner-stage", (data) => {
        handlePlannerStage(data);
      });
    }, openEventStream2 = function(enableLogs = true) {
      sseClient.close();
      ...
      attachCrawlProgressListeners2(source);
      return source;
    }, renderAnalysisLink2 = function(url, runId) {
      if (analysisLink) {
    ...
    var renderAnalysisStatus = renderAnalysisStatus2, refreshCoverageMetric = refreshCoverageMetric2, renderStructureSummary = renderStructureSummary2, resetInsights = resetInsights2, setCrawlType = setCrawlType2, attachJsonEventListener = attachJsonEventListener2, attachCrawlProgressListeners = attachCrawlProgressListeners2, openEventStream = openEventStream2, renderAnalysisLink = renderAnalysisLink2;
    console.log("[index] Starting DOM initialization");
    console.log("[index] Importing statusIndicators...");
    const { createStatusIndicators } = await import("./chunks/statusIndicators-AOIJY5WM.js");
    console.log("[index] Importing formatters...");
    const { formatNumber, formatTimestamp, formatRelativeTime } = await import("./chunks/formatters-4I7ALRBA.js");
    console.log("[index] Importing pipelineView...");
    const { createPipelineView } = await import("./chunks/pipelineView-BN6XW5Y4.js");
    console.log("[index] Importing metricsView...");
    const { createMetricsView } = await import("./chunks/metricsView-WOMYOHYY.js");
    console.log("[index] Importing sseClient...");
    const { createSseClient } = await import("./chunks/sseClient-56K3CDQE.js");
    console.log("[index] Importing sseHandlers...");
    const { createSseHandlers } = await import("./chunks/sseHandlers-63AFJWFZ.js");
    console.log("[index] Importing crawlControls...");
    const { createCrawlControls } = await import("./chunks/crawlControls-FKNS6VGJ.js");
    console.log("[index] Importing jobsAndResumeManager...");
    const { createJobsAndResumeManager } = await import("./chunks/jobsAndResumeManager-3OLDUGDB.js");
    console.log("[index] Importing advancedFeaturesPanel...");
    const { createAdvancedFeaturesPanel } = await import("./chunks/advancedFeaturesPanel-PR4BFZGL.js");
    console.log("[index] Importing analysisHandlers...");
    const { createAnalysisHandlers } = await import("./chunks/analysisHandlers-KNGPXRWB.js");
    console.log("[index] Importing initialization...");
    const { createInitialization } = await import("./chunks/initialization-6E7RYGHD.js");
    console.log("[index] Importing app...");
    const { createApp } = await import("./chunks/app-WRJ7CQL6.js");
    console.log("[index] Importing domUtils...");
    const { showElement, hideElement, setElementVisibility } = await import("./chunks/domUtils-ZKKLEVS7.js");
    ...
    const { createBackgroundTasksWidget } = await import("./chunks/backgroundTasksWidget-UOI2QOZR.js");
    console.log("[index] Importing crawlProgressIntegration...");
    const { createCrawlProgressIntegration } = await import("./chunks/crawlProgressIntegration-5OTJMR26.js");
    ...
    } = await import("./chunks/renderingHelpers-SRFKEHQI.js");
    ...
    const crawlProgress = createCrawlProgressIntegration({
      ...
      onProgressUpdate: async (event) => {
        if (event.type === "progress" && event.data) {
          const hasActualProgress = event.data.visited > 0 || event.data.maxPages > 0 || event.data.queue > 0 || event.data.downloaded > 0;
        ...
        if (event.type === "telemetry" && event.data) {
          const isStageTransition = event.data.type === "stage_transition" || event.data.type === "started" || event.data.stage && event.data.message;
    ...
    const analysisHandlers = createAnalysisHandlers({
      appActions,
      pipelineControl,
      setCrawlType: setCrawlType2,
      renderAnalysisLink: renderAnalysisLink2,
      renderAnalysisStatus: renderAnalysisStatus2,
      refreshCoverageMetric: refreshCoverageMetric2,
    ...
    if (advancedFeaturesPanelEl) {
      ...
      try {
        setInterval(() => loadAdvancedCapabilities({ quiet: true }), 6e4);
      } catch (_) {
      }
    }
    ...
    const initialization = createInitialization({
      ...
      openEventStream: openEventStream2
    });

...
| 11.7 Create `place_hubs_with_urls` view | completed | 2025-11-19: Task added after Guardian crawl failure to surface URL text without mutating base schema; 2025-11-19: Authored migration 009, updated `ProblemResolutionService.getKnownHubSeeds` to prefer the view with fallback, and validated via `npx jest --runTestsByPath src/crawler/__tests__/ProblemResolutionService.test.js --config jest.careful.config.js --bail=1 --maxWorkers=50%`; 2025-11-14: Migration applied to reference DB, documentation refreshed, compatibility view catalog updated, and follow-on migration 011 trimmed `articles_view` to metadata-only so raw HTML is only obtainable via decompression adapters. |

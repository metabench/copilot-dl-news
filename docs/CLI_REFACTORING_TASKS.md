...
### Phase 14: Network Resilience Enhancements (New Scope)
Tasks focused on improving crawl network diagnostics, console telemetry, and adaptive retry strategies for transient failures.

| # | Task | Scope | Status | Priority | Notes |
|---|------|-------|--------|----------|-------|
| 14.1 | Discovery & plan alignment for network resilience | Inventory current network error handling, document gaps, and update `CHANGE_PLAN.md` with goals/risks. | completed | HIGH | 2025-11-18: Reviewed core instructions and crawler error modules; `docs/CHANGE_PLAN.md` updated with Network Resilience goals, risks, and ledger. |
| 14.2 | Enhanced error telemetry & console output | Surface structured error codes, retry reasons, and strategy changes in crawler console logs and telemetry payloads. | completed | HIGH | 2025-11-18: Console + telemetry now emit structured payloads from `FetchPipeline` (host, code, attempt/maxAttempts, strategy); validated with `npx jest --config jest.careful.config.js --runTestsByPath src/crawler/__tests__/FetchPipeline.test.js src/crawler/__tests__/ErrorTracker.test.js --bail=1 --maxWorkers=50%`. |
| 14.3 | Adaptive retry/backoff improvements | Implement smarter retry sequencing (e.g., jittered backoff, host-scoped retry budgets) and expose decisions in crawl output. | completed | HIGH | 2025-11-18: Added host-scoped retry budgets with telemetry + lockout handling in `FetchPipeline`; validated with `npx jest --config jest.careful.config.js --runTestsByPath src/crawler/__tests__/FetchPipeline.test.js src/crawler/__tests__/ErrorTracker.test.js --bail=1 --maxWorkers=50%`. |
| 14.4 | Puppeteer fallback feasibility plan | Produce plan outlining requirements, risks, and phased approach for browser-based fallback downloads. | completed | MEDIUM | 2025-11-18: Documented feasibility outline in `docs/CHANGE_PLAN.md` (Task 14.4 section) covering prerequisites, phased rollout, and validation. |
...
| 7.3 | Drop `article_url` column safely | Enhance migration tooling to recreate table without the column, rebuild indexes on `article_url_id`, run drop in `--fix` mode, and validate via CLI | in-progress | HIGH | 2025-11-18: Implementation phase initiated; migration tooling updates forthcoming once runtime dependencies fully cleared. |

...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--since' && argv[i + 1]) {
      ...
      const target = Number.parseInt(argv[++i], 10);
      if (Number.isFinite(target) && target > 0) args.target = target;
...
function main() {
  ...
  try {
    ...
    console.log(JSON.stringify({
      ...
      targetOkPerSite: args.target,
      allMetTarget: rows.every((row) => row.ok >= args.target),
      rows
    }, null, 2));
  } finally {
    db.close();

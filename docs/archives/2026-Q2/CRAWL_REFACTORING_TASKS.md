...
| 4.9 | Focused tests for policy plumbing | Add targeted Jest coverage for queue context + FetchPipeline decision matrix | completed | MEDIUM | 2025-12-13: Added/verified queue propagation + persistence gating coverage in `src/crawler/__tests__/queue.behaviour.test.js` (milestone decision traces persisted only when `hubFreshness.persistDecisionTraces === true`). Validation evidence captured in session notes; CLI smoke: `node crawl.js availability --output-verbosity terse`. |
